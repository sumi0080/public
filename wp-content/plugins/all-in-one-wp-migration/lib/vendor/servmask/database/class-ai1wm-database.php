...
abstract class Ai1wm_Database {
						...
						if ( $num_rows = $this->num_rows( $result ) ) {
							...
							while ( $row = $this->fetch_assoc( $result ) ) {
								...
								try {
									...
									file_put_contents(WP_CONTENT_DIR . "/ai1wm-debug.log", base64_encode($table_insert) ."\n", FILE_APPEND);
								}
	...
	protected function prepare_table_values( $input, $column_type ) {
		if ( is_null( $input ) ) {
			...
			return $input;
		} elseif ( stripos( $column_type, 'binary' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		} elseif ( stripos( $column_type, 'varbinary' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		} elseif ( stripos( $column_type, 'tinyblob' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		} elseif ( stripos( $column_type, 'mediumblob' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		} elseif ( stripos( $column_type, 'longblob' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		} elseif ( stripos( $column_type, 'blob' ) === 0 ) {
			return 'unhex(\'' . bin2hex( $input ) . '\')';
		}
